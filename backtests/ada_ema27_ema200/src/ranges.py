...
def detect(df: pd.DataFrame, w: int = W, min_bars: int | None = None,
    ...
    while i < n:
        ...
        # Цена обязана быть внутри границ на баре открытия. Без этой проверки
        # внутренний скан останавливается сразу, но коридор всё равно
        # записывается от t0 до i — то есть коробка, которая ни одного бара
        # цену не содержала. Дефект вскрыт побарным эмулятором Pine
        # (src/pine_emu2.py): Pine так не умеет, и версии разошлись.
        if not (bot <= c[i] <= top):
            i += 1
            continue
