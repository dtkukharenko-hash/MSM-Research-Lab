...
# Прямоугольники, размеченные владельцем на 4h. Даты сняты с ada_4h_3.png
# по осям, точность +/- пара баров. До уточнения владельцем — ориентир, не эталон.
BOXES = {
    "A": ("2023-11-22", "2023-12-05"),
    "B": ("2023-12-13", "2024-01-04"),
...
def sweep(df: pd.DataFrame, widths: tuple[int, ...]) -> None:
    ...
    head = "  ".join(f"{k:>10}" for k in BOXES)
    print(f"{'w':>3} {'коридоров':>10} {'покрытие':>9} {'баров мед':>10} "
          f"{'ширина мед':>11}   перекрытие с боксом (осколков)")
    print(f"{'':>3} {'':>10} {'':>9} {'':>10} {'':>11}   {head}")
    for w in widths:
        z = detect(df, w=w)
        if z.empty:
        ...
        for k, (a, b) in idx.items():
            ...
            if hit.empty:
                cells.append(f"{'—':>10}")
                continue
            ov = ((np.minimum(b, hit.end_idx) - np.maximum(a, hit.start_idx) + 1)
                  / (b - a + 1) * 100)
            cells.append(f"{ov.max():7.0f}% ({len(hit)})")
