...
run_fixture() {
  ...
  for path in "$tmp/state" "$tmp/state"/{queue,running,completed,blocked,failed,logs,locks}; do
...
for f in msm_orchestrator.py msm_worker.sh install_orchestrator.sh verify_orchestrator.sh; do
  [[ -f "$REPO/automation/$f" ]] || { echo "missing $f" >&2; exit 1; }
done
PYTHONPYCACHEPREFIX="$PYCACHE_ROOT" python3 -m py_compile "$REPO/automation/msm_orchestrator.py"
...
install -m 644 "$REPO/automation/msm_orchestrator.py" "$REPO/automation/msm_worker.sh" "$DEST/"
install -m 644 "$REPO/automation/msm-orchestrator.service" "$UNIT/msm-orchestrator.service"
for d in "$STATE" "$STATE/queue" "$STATE/running" "$STATE/completed" "$STATE/blocked" "$STATE/failed" "$STATE/logs" "$STATE/locks"; do
