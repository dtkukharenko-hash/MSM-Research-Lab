...
[[ $(field status) == READY ]] || fail "AUTOMATION-004 is not READY"
...
[[ -x "$REPO/automation/install_orchestrator.sh" ]] || fail "installer missing or not executable"
[[ -x "$REPO/automation/verify_orchestrator.sh" ]] || fail "verifier missing or not executable"

echo "[3/7] Static verification"
bash -n "$REPO/automation/"*.sh
python3 -m py_compile "$REPO/automation/msm_orchestrator.py"
"$REPO/automation/verify_orchestrator.sh" --offline

echo "[4/7] Install in test mode"
"$REPO/automation/install_orchestrator.sh" --install --test-mode
systemctl daemon-reload
systemctl enable --now "$NEW_SERVICE"
"$REPO/automation/verify_orchestrator.sh" --service --test-mode --wait 90

echo "[5/7] Full mock state-machine cycle"
"$REPO/automation/verify_orchestrator.sh" --mock-cycle --wait 180

# Switch only after every test above succeeds.
echo "[6/7] Atomic production switch"
systemctl stop "$NEW_SERVICE"
"$REPO/automation/install_orchestrator.sh" --activate-production
systemctl enable --now "$NEW_SERVICE"
"$REPO/automation/verify_orchestrator.sh" --service --production --wait 90
...
"$REPO/automation/verify_orchestrator.sh" --health
