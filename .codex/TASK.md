...
- task_id: `EXP-031-TEMPORAL-VALIDATION-DATASET`
- status: `READY`
- target_branch: `main`
...
Create a genuinely untouched temporal validation diagnostic dataset for calendar year 2025 using the exact frozen EXP-027 and EXP-029R causal protocol. This experiment prepares data only. It must not test, rank, select, confirm or reject the 226 volatility-conditioned cells found in EXP-030R; that confirmation belongs to a later preregistered experiment.

## Frozen sources, panel and period

Use the committed protocol and outputs from EXP-027, EXP-029R and EXP-030R only as frozen definitions and integrity references.
...
7. chronological thirds, recalculated only as equal chronological thirds of the new 2025 interval;
...
Before accepting the 2025 output, prove that the implementation reproduces the committed protocol on a frozen overlap probe:
...
## Temporal validation dataset contract

`validation_observations.csv.gz` must contain one scalar row per 2025 event representative and matched control with the same committed schema as EXP-029R observations, including:
...
`validation_volatility_state.csv` must preserve the same identity key, causal state fields, regime labels and explicit reasons as EXP-029R. `UNKNOWN` is never imputed.

## Validation gates

Require all of the following for `TEMPORAL_VALIDATION_DATASET_READY`:
...
- `TEMPORAL_VALIDATION_DATASET_READY` — complete 2025 dataset and all validation gates pass;
- `TEMPORAL_VALIDATION_DATASET_PARTIAL` — usable data exist, but explicit source limitations remain and are fully retained;
- `TEMPORAL_VALIDATION_DATASET_DATA_FAILED` — source integrity, reconciliation or causal construction is insufficient.
...
- `experiments/EXP-031_TEMPORAL_VALIDATION_DIAGNOSTIC_DATASET/REPORT.md`
- `experiments/EXP-031_TEMPORAL_VALIDATION_DIAGNOSTIC_DATASET/data_provenance.csv`
- `experiments/EXP-031_TEMPORAL_VALIDATION_DIAGNOSTIC_DATASET/validation_observations.csv.gz`
- `experiments/EXP-031_TEMPORAL_VALIDATION_DIAGNOSTIC_DATASET/validation_volatility_state.csv`
- `experiments/EXP-031_TEMPORAL_VALIDATION_DIAGNOSTIC_DATASET/protocol_reconciliation.csv`
- `experiments/EXP-031_TEMPORAL_VALIDATION_DIAGNOSTIC_DATASET/coverage_summary.csv`
- `experiments/EXP-031_TEMPORAL_VALIDATION_DIAGNOSTIC_DATASET/validation_summary.csv`
- `experiments/EXP-031_TEMPORAL_VALIDATION_DIAGNOSTIC_DATASET/counterexamples.csv`
- `experiments/EXP-031_TEMPORAL_VALIDATION_DIAGNOSTIC_DATASET/run_hashes.csv`
...
Planner, implementer, auditor and corrector use the required JSON role contract. The implementer leaves only the ten allowlisted EXP-031 outputs unstaged. The orchestrator performs final baseline-relative validation, commits once using the task identifier and pushes to `main`.