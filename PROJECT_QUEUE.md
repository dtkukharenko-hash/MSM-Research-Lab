...
Joint candidates: `5`, confirmed: `3`, failed: `2`.
...
Вердикт: PARTIAL_ENTRY_STRUCTURE — создана ретроспективная reference-разметка 12 крупных движений ADAUSDT
4H за 2023-07-01 → 2024-12-31 без расчёта PnL, без backtest и без данных 2025–2026. Для каждого движения
размечено максимум два разрешённых входа: 12 `PRIMARY_ENTRY`, 12 `OPTIONAL_SECONDARY_ENTRY`, а также 36
`BLOCKED_EXAMPLES`. Диагностика EXP-007 показала, что 65.3% сигналов, попавших внутрь крупных движений,
возникали в зрелой/поздней/истощающей фазе; EMA27/EMA200 полезны для направления, но недостаточны для
тайминга входа внутри уже идущего движения. Детали:
`experiments/EXP-008_MAJOR_MOVE_ENTRY_LABELING/REPORT.md`.

Цель:
Создать reference-разметку правильных и неправильных мест входа внутри крупных EMA-движений без генерации
новых сделок и без оптимизации.

Исполнитель:
Codex

Результат:
REPORT.md, CSV-артефакты, Pine-разметчик и обзорный PDF созданы; причинный live-детектор начала движения
